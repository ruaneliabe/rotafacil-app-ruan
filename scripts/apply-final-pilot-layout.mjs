...
// Controle da operação em lugar estável: no cabeçalho, ao lado do Cardápio Web.
// Também corrige a fila: motoboy chamado ao balcão deixa de ocupar posição na fila imediatamente.
patch('src/components/StoreDashboardLegacy.tsx', (input) => {
  ...
  // Evita duplicidade com qualquer card legado de operação na sidebar.
  s = s.replace(/<div data-sidebar-operation-card="true" className=\{`[^`]*`\}>/, '<div data-sidebar-operation-card="true" className="hidden">');

  // Motoboy que já foi chamado ao balcão não continua concorrendo pela 1ª posição da fila.
  s = s.replace(
    ".filter((m) => m.status === 'available')",
    ".filter((m) => m.status === 'available' && (!m.callingToCounterAt || Number(m.callingToCounterAt) < Number(m.joinedQueueAt || 0)))"
  );

  // Ao chamar, grava a chamada e garante que ele deixe a fila imediatamente.
  s = s.replaceAll(
...
// No app do motoboy, a fila usa a mesma regra do painel da loja.
// Assim o chamado deixa de ser '1º da fila' e o próximo sobe instantaneamente.
...
// "Em entrega" vira visão secundária expansível.
