...
export function RuntimeCorrections() {
  useEffect(() => {
    ...
    const apply=()=>{
      ...
      Array.from(document.querySelectorAll('p,span,div,h3')).forEach(el=>{if(el.children.length)return;if(el.textContent?.trim()==='Motoboy reservado')el.textContent='Pedidos vinculados a motoboy';if(el.textContent?.trim()==='⚙️ Gestão & Fechamento')el.textContent='💰 Financeiro'});
