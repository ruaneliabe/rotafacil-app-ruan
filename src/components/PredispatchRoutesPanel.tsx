          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xs font-black text-slate-900">
                Pico de pedidos: <span className="text-amber-700">{looseOrders.length} soltos</span> • {prepared.length} rotas montadas • {queueDrivers.length} motoboy{queueDrivers.length === 1 ? '' : 's'} livre{queueDrivers.length === 1 ? '' : 's'}
              </div>
      ...
      <div className="grid min-w-0 grid-cols-1 gap-3 xl:grid-cols-[1.05fr_1.25fr_1.05fr_.78fr]">
        <section className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-100 px-3.5 py-3">
            <div><h3 className="text-sm font-black text-slate-900">Pedidos soltos</h3><p className="text-[10px] text-slate-400">Ainda não agrupados em rota</p></div>
          ...
          <div className="max-h-[760px] min-h-[620px] space-y-2 overflow-y-auto p-2.5">
            {looseOrders.length ? looseOrders.map((order) => {
              ...
              return <article key={order.id} className={`rounded-xl border bg-white p-3 shadow-sm ${wait >= 20 ? 'border-rose-200' : 'border-slate-200'}`}>
                <div className="flex items-start justify-between gap-2"><div><div className="flex items-center gap-1.5"><b className="text-xs text-slate-950">{code(order)}</b>{wait >= 20 && <span className="rounded-full bg-rose-50 px-1.5 py-0.5 text-[8px] font-black text-rose-600">ATENÇÃO</span>}</div><p className="mt-1 text-[11px] font-semibold text-slate-700">{order.clientName}</p></div><span className={`text-[9px] font-black ${wait >= 20 ? 'text-rose-600' : 'text-slate-400'}`}>Há {wait} min</span></div>
                <p className="mt-2 flex gap-1.5 text-[9px] text-slate-500"><MapPin className="mt-0.5 h-3 w-3 shrink-0" />{order.address}</p>
                <div className="mt-2 flex items-center justify-between"><span className={`rounded-full px-2 py-1 text-[8px] font-black ${isReady(order) ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>{isReady(order) ? 'Pronto' : 'Preparando'}</span></div>
              </article>;
        ...
        <section className="min-w-0 overflow-hidden rounded-2xl border border-violet-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-100 px-3.5 py-3">
            <div className="flex items-center gap-2"><span className="grid h-8 w-8 place-items-center rounded-lg bg-violet-50 text-violet-600"><Route className="h-4 w-4" /></span><div><h3 className="text-sm font-black text-slate-900">Rotas montadas</h3><p className="text-[10px] text-slate-400">Cargas penduradas aguardando liberação</p></div></div>
            <span className="rounded-full bg-violet-50 px-2 py-1 text-xs font-black text-violet-700">{prepared.length}</span>
          </div>
          <div className="max-h-[760px] min-h-[620px] space-y-2 overflow-y-auto p-2.5">
