...
export const StoreAccountSettingsModal: React.FC<StoreAccountSettingsModalProps> = ({isOpen,onClose,shift,onSaveSettings,firstSetup=false}) => {
  ...
  const [explicitlyOpened,setExplicitlyOpened]=useState(!firstSetup);

  useEffect(()=>{
    const onDocumentClick=(event:MouseEvent)=>{
      const target=(event.target as HTMLElement|null)?.closest('button,a');
      const label=(target?.textContent||'').trim().toLowerCase();
      if(label==='configurações' || label.includes('configurações da loja')) setExplicitlyOpened(true);
    };
  ...
  // setupRequired pode vir marcado em dados antigos do Firestore. Isso não deve
  // bloquear o painel nem abrir o modal sozinho. Só mostramos configurações
  // quando o usuário realmente clicar em Configurações.
  if(!isOpen || (firstSetup && !explicitlyOpened))return null;

  const closeModal=()=>{setExplicitlyOpened(false);onClose();};
  ...
  return <div className="fixed inset-0 z-50 bg-slate-950/35 backdrop-blur-[2px] flex items-center justify-center p-4 overflow-y-auto" onMouseDown={(e)=>{if(e.target===e.currentTarget)closeModal()}}>
    <div className="bg-white text-slate-900 rounded-2xl max-w-xl w-full shadow-2xl border border-slate-200 overflow-hidden my-6">
      <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between bg-white">
        ...
        <button type="button" onClick={closeModal} className="w-8 h-8 grid place-items-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-700"><X className="w-4 h-4"/></button>
      </div>
