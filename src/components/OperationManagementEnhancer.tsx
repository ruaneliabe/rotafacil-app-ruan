...
export const OperationManagementEnhancer: React.FC<Props> = ({
  ...
  useEffect(() => {
    const sync = () => {
      ...
      if (!heading) {
        if (legacyOverlayRef.current && !legacyOverlayRef.current.isConnected) {
  ...
  const close = () => {
    const closeButton = legacyCloseRef.current;
    if (legacyOverlayRef.current?.isConnected) legacyOverlayRef.current.style.display = '';
    setActive(false);
    legacyOverlayRef.current = null;
    legacyCloseRef.current = null;
  ...
  return createPortal(
    <div data-operation-enhanced-modal="true" className="fixed inset-0 z-[90] bg-slate-950/45 p-3 md:p-5 flex items-center justify-center">
      ...
      <section className="flex h-[92vh] w-full max-w-[1540px] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-[#FAF9F6] shadow-2xl">
        <div className="flex flex-col gap-3 border-b border-slate-200 bg-white px-5 py-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <h2 className="text-xl font-black tracking-tight text-slate-950">Gestão de entrega</h2>
          ...
          <div className="flex items-center gap-3">
            <span className="inline-flex items-center gap-1.5 text-[11px] text-slate-500"><span className="h-2 w-2 rounded-full bg-emerald-500" />Atualizado agora</span>
            <button onClick={onOpenNewOrderModal} className="inline-flex items-center gap-2 rounded-xl bg-violet-600 px-3.5 py-2.5 text-xs font-black text-white hover:bg-violet-500"><Zap className="h-4 w-4" />Novo despacho</button>
            <button onClick={close} className="grid h-9 w-9 place-items-center rounded-lg border border-slate-200 bg-white text-slate-500 hover:bg-slate-50"><X className="h-4 w-4" /></button>
          </div>
        </div>
          ...
          {metrics.map(([label, value, helper, Icon, cls]) => (
            <div key={label} className="rounded-xl border border-slate-200 bg-white p-3">
              <div className="flex items-center gap-2">
                <span className={`flex h-9 w-9 items-center justify-center rounded-xl ${cls}`}><Icon className="h-4 w-4" /></span>
                <div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-4"><span className="text-xs font-black text-slate-700">{label}</span><strong className="text-xl text-slate-950">{value}</strong></div><p className="mt-0.5 text-[10px] text-slate-500">{helper}</p></div>
              </div>
            </div>
          ))}
        </div>
