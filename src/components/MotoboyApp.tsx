...
export const MotoboyApp: React.FC<Props> = ({
  ...
  const addressForNav = (order: Order) => order.lat && order.lng ? `${order.lat},${order.lng}` : order.address;
  ...
  const openGoogle = () => {
    ...
    const selectedRoute = navRequest.fullRoute ? remainingRoute(navRequest.from) : remainingRoute(navRequest.from).slice(0, 1);
  ...
  const openWaze = () => {
    ...
    if (!target) return;
    const destination = target.lat && target.lng ? `ll=${target.lat},${target.lng}` : `q=${encodeURIComponent(target.address)}`;
    setNavRequest(null);
  ...
  return (
    <div className="mx-auto flex min-h-[100dvh] w-full max-w-md flex-col overflow-hidden bg-[#F4F6F9] text-slate-950 sm:min-h-[780px] sm:rounded-[28px] sm:border sm:border-slate-200 sm:shadow-2xl">
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <div className="mt-1 flex items-center gap-1.5 text-[12px] font-medium text-slate-300">
              ...
              <span>Online</span><span className="text-slate-500">•</span><span>{statusText}</span>
            </div>
          </div>
      ...
      <section className="grid grid-cols-3 divide-x divide-slate-200 border-b border-slate-200 bg-white px-3 py-4">
        ...
        <div className="text-center"><Package className="mx-auto h-5 w-5 text-[#0A1C31]"/><div className="mt-1 text-[21px] font-black leading-none">{completedToday.length}</div><p className="mt-1 text-[10px] font-bold text-slate-500">entregues</p></div>
      ...
      <main className="flex-1 space-y-3 px-3 pb-6">
        ...
        </> : <section className="rounded-2xl border border-slate-200 bg-white px-6 py-10 text-center shadow-sm"><div className="mx-auto grid h-24 w-24 place-items-center rounded-full bg-gradient-to-br from-violet-50 to-slate-50"><Route className="h-12 w-12 text-violet-600"/></div><h2 className="mt-5 text-[22px] font-black tracking-tight">Sua rota está vazia.</h2><p className="mx-auto mt-2 max-w-[270px] text-[13px] leading-relaxed text-slate-500">Os pedidos aceitos aparecerão aqui na ordem de entrega.</p><div className="mt-6 flex items-start gap-2 rounded-xl bg-violet-50 p-4 text-left text-[11px] leading-relaxed text-violet-700"><Info className="mt-0.5 h-4 w-4 shrink-0"/><span>Assim que você retirar um pedido, ele será adicionado automaticamente à sua rota.</span></div></section>)}

        {tab === 'history' && <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-100 px-4 py-4">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2"><CalendarDays className="h-5 w-5 text-[#081A2F]"/><div><h2 className="text-[17px] font-black">Entregas</h2><p className="text-[10px] text-slate-500">Consulte seus dias anteriores</p></div></div>
            ...
            <div className="mt-4 grid grid-cols-[40px_1fr_40px] items-center gap-2">
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-center"><p className="text-[13px] font-black text-slate-900">{historyDateLabel(historyOffset)}</p><p className="mt-0.5 text-[9px] font-bold uppercase tracking-wide text-slate-400">{dateFromOffset(historyOffset).toLocaleDateString('pt-BR',{weekday:'long',day:'2-digit',month:'2-digit'})}</p></div>
          ...
          <div className="divide-y divide-slate-100">
            {historyOrders.length ? historyOrders.map((order) => <div key={order.id} className="flex w-full items-center gap-3 px-4 py-3"><CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-500"/><div className="min-w-0 flex-1"><p className="truncate text-[12px] font-bold text-slate-800">#{order.codeNumber} <span className="mx-1 text-slate-300">•</span> {order.clientName}</p><p className="mt-0.5 truncate text-[10px] text-slate-400">{order.neighborhood || order.address}</p></div><span className="text-[10px] font-bold text-slate-400">{formatTime(order)}</span></div>) : <div className="px-5 py-12 text-center"><History className="mx-auto h-9 w-9 text-slate-300"/><p className="mt-3 text-[13px] font-bold text-slate-500">Nenhuma entrega neste dia.</p><p className="mt-1 text-[10px] text-slate-400">Use as setas acima para consultar outra data.</p></div>}
          </div>
        </section>}
      </main>
