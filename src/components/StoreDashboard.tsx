...
export const StoreDashboard: React.FC<any> = (props) => {
  ...
  useEffect(() => {
    if (optimisticOpen !== null && cloudOpen === optimisticOpen) setOptimisticOpen(null);
  ...
  const handleToggleOperation = async () => {
    ...
    try {
      if (nextOpen) {
        await setDoc(doc(db, 'shifts', 'current_shift'), {
          isOpen: true,
          setupRequired: false,
          pilotMode: true,
          demoDataDisabled: true,
          shiftDate: today,
          shiftId: sameDayOperation ? props.shift.shiftId : `shift_${today}_${nowTimestamp}`,
          openedAt: sameDayOperation ? (props.shift.openedAt || nowTime) : nowTime,
          openedTimestamp: sameDayOperation ? (props.shift.openedTimestamp || nowTimestamp) : nowTimestamp,
          closedAt: null,
  ...
  const effectiveShift = { ...(props.shift || {}), isOpen: operationOpen };
  const dashboardProps = { ...props, shift: effectiveShift, onToggleShift: handleToggleOperation };
    ...
    return (
      <div className="flex min-h-[calc(100vh-32px)] w-full items-center justify-center rounded-2xl border border-slate-200 bg-white/70 shadow-sm backdrop-blur-sm">
        <div className="flex flex-col items-center gap-3 text-center">
          <div className="grid h-12 w-12 place-items-center rounded-2xl bg-violet-50 text-violet-600"><Loader2 className="h-6 w-6 animate-spin" /></div>
          <div><p className="text-sm font-black text-slate-900">Sincronizando operação</p><p className="mt-1 text-[11px] text-slate-500">Carregando pedidos, entregadores e status da loja...</p></div>
        </div>
      </div>
    );
  }
        ...
        <div className={`rounded-xl border bg-white/95 p-2 shadow-[0_8px_22px_rgba(15,23,42,.08)] backdrop-blur ${operationOpen ? 'border-emerald-200' : 'border-slate-200'}`}>
          <div className="flex items-center gap-2">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1"><span className={`h-1.5 w-1.5 rounded-full ${operationOpen ? 'bg-emerald-500' : 'bg-slate-400'}`} /><p className="truncate text-[9px] font-black text-slate-900">{operationOpen ? 'Operação aberta' : 'Operação fechada'}</p></div>
